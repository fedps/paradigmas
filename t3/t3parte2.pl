...
%4. Defina um predicado has5(L) que seja verdadeiro se L for uma lista de 5 elementos. Lembre de como funciona a 
%   unificação em Prolog e resolva este exercício 
...
%5. Defina um predicado hasN(L,N) que seja verdadeiro se L for uma lista de N elementos. Agora você pode usar um predicado 
%   pré-definido.
%   ?- hasN([a,b,c],3).
%      true.
%   ?- hasN([a,b,c,d,e],3).
...
%7. Defina um predicado mesmaPosicao(A,L1,L2) para verificar se um elemento A está na mesma posição nas listas L1 e L2. 
%   Assuma que L1 e L2 sejam permutações de uma lista de elementos distintos, sem repetições. Dica: procure auxílio 
%   em predicados pré-definidos. Exemplo de uso:
...
%10. Defina um predicado recursivo repete(N,E,L) que seja verdadeiro se a lista L for composta por exatamente N repetições 
%    do elemento E.
repete(0,_,[]).
repete(N,E,L) :- L = [H|T],
		 length(L,N),
...
%11. Defina um predicado recursivo mapop(L1,L2) que seja verdadeiro se a lista L2 for formada aplicando-se a operação 
%    2*x+1 a cada elemento x da lista L1.
%    ?- mapop([1,2,3],L).
%       L = [3, 5, 7] .
%    ?- mapop([],L).
%       L = [] .
